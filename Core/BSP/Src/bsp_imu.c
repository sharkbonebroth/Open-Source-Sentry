...
mpu_data_t            mpu_data;
imu_t				  imu={0};
gimbal_heading_t			gimbal_heading;
...
uint8_t mpu_read_bytes(uint8_t const regAddr, uint8_t* pData, uint8_t len)
{
    ...
    tx_buff[0] = tx;
    HAL_SPI_TransmitReceive(&MPU_HSPI, &tx, &rx, 1,100);
    HAL_SPI_TransmitReceive(&MPU_HSPI, tx_buff, pData, len,100);
    MPU_NSS_HIGH;
    return 0;
}
...
{
    mpu_read_bytes_DMA(MPU6500_EXT_SENS_DATA_00, buff, 6);
}
...
uint8_t imu_init(void)
{
	...
	uint8_t MPU6500_Init_Data[9][2] = {{ MPU6500_PWR_MGMT_1, 		0x03 },		// Clock Source - auto select???
										...
										{ MPU6500_CONFIG, 			0x02 },		// Gyro and Temp LPF 2 @ 92Hz */
										{ MPU6500_GYRO_CONFIG, 		0x18 },		// Gyro +-2000dps */
										{ MPU6500_ACCEL_CONFIG, 	0x10 },		// +-8G */
										{ MPU6500_ACCEL_CONFIG_2, 	0x02 },		// Accl LPF 2 @ 92Hz */
...
void imu_attitude_update(void)
{
	...
	else if(IMU_ORIENTATION == 1)
	{
		...
		imu.yaw = -atan2(2*q1*q2 + 2*q0*q3, -2*q2*q2 - 2*q3*q3 + 1);
		/* pitch  -pi/2----pi/2 */
		imu.rol = -asin(-2*q1*q3 + 2*q0*q2);
		/* roll   -pi----pi  */
		imu.pit =  atan2(2*q2*q3 + 2*q0*q1, -2*q1*q1 - 2*q2*q2 + 1);
	}
